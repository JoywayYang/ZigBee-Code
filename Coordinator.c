...
static void GenericApp_SendTheMessage( void )
{
  ...
  osal_memcpy(&theMessageData,&uartbuf,sizeof(uartbuf));
  afAddrType_t my_DstAddr; //设备描述符
  my_DstAddr.addrMode = (afAddrMode_t)AddrBroadcast;//网络内广播
  my_DstAddr.endPoint = GENERICAPP_ENDPOINT;
  my_DstAddr.addr.shortAddr = 0xFFFF; //发送到所有设备
...
{  
  ...
  {  
   ...
   HalUARTRead(port, uartbuf,sizeof(uartbuf)); //读取串口数据
   //设置自定义的事件发生。
